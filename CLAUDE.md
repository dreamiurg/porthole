...
| `site/`, `tools/build_site.py` | The web installer on GitHub Pages (`https://dreamiurg.net/porthole/`), built from each app's latest release and its `app.json`. ESP Web Tools does the flashing. The page is a plain personal one-pager in English and Ukrainian (every visible string has both `lang` spans): no marketing layout, no big call-to-action buttons. |
