...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: hand-placed pixel art; each branch is one time-of-day or stage detail
function room(ctx, timeOfDay) {
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: hand-placed pixel art; each branch is one pose or stage detail
function puppy(ctx, x, y, { sleeping, blink, wag, activity, stage, pose, phase }) {
  let headX = 0;
  let headY = 0;
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: per-pose pixel transform for spin and roll
  ...
  headX = pose === 'bow' ? -6 : 0;
  headY = pose === 'bow' ? 5 + phase * 3 : pose === 'sit' ? -3 : 0;
  ...
  if (!facingBack) {
    ...
    r(INK, -4, 12, 4, 1);
    r(INK, 2, 12, 4, 1);
    if (!sleeping && activity !== 'read') r('#d67584', 0, 12, 2, 3);
  }
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: scene composition; each branch is one activity overlay
export function renderScene(ctx, { time = 0, state = {}, activity = 'idle', fetchTarget = null, fetchScore = 0, reducedMotion = false, timeOfDay = 'day' } = {}) {
  ...
  else room(ctx, sleeping ? 'night' : timeOfDay);
  const bob = !sleeping && tick % 1600 > 800 ? 1 : 0;
  ...
  const phase = reducedMotion ? (pose === 'roll' ? 2 : 1) : Math.floor(tick / (turning ? 230 : 400)) % (turning ? 4 : 2);
  const jump = pose === 'jump' ? (reducedMotion ? 11 : [0, 3, 7, 11, 7, 3][Math.floor(tick / 130) % 6]) : 0;
  rect(ctx, '#947d7b', 64 + Math.floor(jump / 3), 110, 35 - Math.floor(jump / 3) * 2, 3);
  puppy(ctx, 80, 80 + bob - jump, { sleeping, blink: tick % 5300 > 5100, wag: !sleeping && tick % 600 < 300, activity, stage, pose, phase });
  if (pose === 'spin') {
  ...
  if (sleeping) {
    ...
    rect(ctx, INK, 67, 109, 28, 3);
    rect(ctx, '#ce8e52', 71, 108, 20, 2);
    rect(ctx, '#7baaa5', 68, 112, 26, 5);
    rect(ctx, '#5a8e8c', 71, 117, 20, 2);
    rect(ctx, '#b5d7c5', 70, 112, 22, 1);
