...
int main() {
  const uint32_t day0 = 1790000000u - (1790000000u % 86400u) + 10 * 3600;  // 10:00
  Save s; pet::adopt(s, day0, "Sam", "Biscuit");
  assert(pet::valid(s));
  ...
  uint32_t t = day0 + 3 * 86400; s.asleep = 0; s.food = 50; int d = 0;
  ...
  // turn taking: a 6 minute session expires, then a rest holds for 10 minutes
  { Save r = s; r.playSec = pet::SESSION_SEC - 1; assert(!pet::sessionExpired(r)); r.playSec = pet::SESSION_SEC; assert(pet::sessionExpired(r));
    r.restUntil = t + pet::REST_SEC; assert(pet::resting(r, t + 1)); assert(!pet::resting(r, t + pet::REST_SEC)); }
