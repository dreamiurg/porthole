...
- `make test`: builds and runs `host/test_pet.cpp` (against `os/*.cpp` and `pet.cpp`), `host/test_shell.cpp` (against `shell/profiles.cpp`, `shell/migrate.cpp` and `pet.cpp`), `host/test_biscuit.cpp` (Biscuit's rules, `games/biscuit/pet.h`), `host/test_gfx565.cpp` (the RGB565 surface and fonts) and `host/test_biscuit_content.cpp` (Biscuit's content gate: every string, its tokens filled with the widest names, measured and drawn by `os/font.cpp` in the box `games/biscuit/layout.h` gives its screen, inside the round glass). Must stay green.
