...
`os/`, `shell/` and each game under `games/<game>/` are the platform-agnostic core. They compile unchanged into both the firmware and the host simulator (`-DHOST_BUILD` only changes what `main()` exists, not the game logic). All are on the include path, so headers include each other by bare name. `os/` never includes anything from `shell/`, `games/` or `firmware/`; games include only `os/`; the shell knows games only through `os/app.h` (one exception: `shell/migrate.cpp` reads a game's old save layout once, for a one-time migration -- Pets Club's old houses today, Biscuit's old NVS blob once its migration lands).

In `shell/`:

- `profiles.h` / `profiles.cpp`: the profile record (`porthole/p<id>`, append-only with magic/version/size/crc, `MAX_PROFILES = 4`), create/delete (delete erases `s<id>` in every game's namespace), the rest budget (`SESSION_SEC`/`REST_SEC`, only with 2+ profiles), and the `Store` interface both platforms implement. `host/test_shell.cpp` exercises it directly. Landing alongside Biscuit: a 25-minute daily play cap per profile, counted across every game (Pets Club included) and reset at the next local midnight -- see the spec's section C for the exact `Record` fields and reset rule.
- `migrate.cpp`: first boot on a Pets Club device: house `crago/s<n>` becomes profile `n` (same slot, no save moves; the older `save` key is copied to `s0` only when no house exists). Runs until the marker `porthole/m` is written, so power loss mid-migration just re-runs it. Landing with Biscuit: a second, independent migration converts a device's old standalone-Biscuit save (NVS namespace `zoegotchi`, key `pet3`) into a Biscuit profile save the same way.
...
Note on constraint 2 and content: each game's content gate is the source of truth, not a character count -- Pets Club's `games/pets-club/tools/check_content.py` re-renders every string through the real 8x8 font and word-wraps it into the exact box the game draws it in; Biscuit's equivalent (landing with its PRs) does the same against its bundled fonts' advance widths and kerning, and additionally rejects any glyph outside the font's set. See the `content` skill.
...
- `make lint`: app-specific static checks: `games/pets-club/tools/check_content.py`, `games/pets-club/tools/art.py --check`, `tools/shell_art.py --check`, and a `-Werror` host build of the shell, the game and the sim. (ruff and shellcheck run from the repo-root pre-commit, not here.) Landing with Biscuit: its own content gate and art-freshness check join this list.
...
- `make art`: runs `games/pets-club/tools/art.py` and `tools/shell_art.py`, regenerating `games/pets-club/sprites.h` and `shell/shell_sprites.h` (and, if Pillow is installed, preview sheets under `build/art/`). Both have `--check`, which `make lint` runs. Landing with Biscuit: its own Node-based art tool (`games/biscuit/tools/art/`) joins this target, generating `games/biscuit/generated/scenes.h` and `discovery_art.h` from committed JS sources -- see the `art` skill.
