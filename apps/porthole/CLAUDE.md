...
Porthole is the device firmware: a shared runtime (`os/`), the board layer (`firmware/`), the shell (`shell/`: "Who's playing?", up to four profiles, the launcher, the rest timer) and the games compiled into one image (`games/`). Its first game is Pets Club. A second game, Biscuit, is being rewritten onto this same runtime (it previously shipped as a standalone LVGL firmware and was removed in PR #24); until its PRs land, everything about it here is design intent, not working code -- read it as "landing with the Biscuit PRs, see the spec." Design: `docs/superpowers/specs/2026-09-24-launcher-profiles-design.md` (shell/profiles) and `docs/superpowers/specs/2026-09-25-biscuit-on-porthole-design.md` (Biscuit) at the repo root.
...
- `migrate.cpp`: first boot on a Pets Club device: house `crago/s<n>` becomes profile `n` (same slot, no save moves; the older `save` key is copied to `s0` only when no house exists). Runs until the marker `porthole/m` is written, so power loss mid-migration just re-runs it. Landing with Biscuit: a second, independent migration converts a device's old standalone-firmware save into a Biscuit profile save the same way.
...
- `games/biscuit/` (landing with the Biscuit PRs): the RGB565 game. Brief: `games/biscuit/CLAUDE.md`.
...
`host/` is the simulator and the tests: `sim.cpp` (the whole shell; SDL2 window, or headless `--script`/`--serve` modes; saves go to `build/host/<namespace>-<key>.sav`), `test_pet.cpp` (the Pets Club simulation self-check) and `test_shell.cpp` (profiles, migration, delete, rest budget, daily cap). Landing with Biscuit: `test_biscuit.cpp`, its own simulation self-check -- a new game's host test always lives here, as `host/test_<name>.cpp`, not inside the game's own directory. None of them touches `firmware/`.
...
- `make lint`: app-specific static checks: `games/pets-club/tools/check_content.py`, `games/pets-club/tools/art.py --check`, `tools/shell_art.py --check`, and a `-Werror` host build of the shell, the game and the sim. (ruff and shellcheck run from the repo-root pre-commit, not here.) Landing with #33: `games/biscuit/tools/check_content.py`, and (skipped with a message where `node` is missing; CI always has it) its art tool's `--check`.
...
- `make test`: builds and runs `host/test_pet.cpp` (against `os/*.cpp` and `pet.cpp`) and `host/test_shell.cpp` (against `shell/profiles.cpp`, `shell/migrate.cpp` and `pet.cpp`). Pure logic self-checks, no rendering. Must stay green.
...
- `make art`: runs `games/pets-club/tools/art.py` and `tools/shell_art.py`, regenerating `games/pets-club/sprites.h` and `shell/shell_sprites.h` (and, if Pillow is installed, preview sheets under `build/art/`). Both have `--check`, which `make lint` runs. Landing with #33: Biscuit's Node-based art tool (`games/biscuit/tools/art/`) joins this target, generating `games/biscuit/generated/scenes.h`, `discovery_art.h` and `art_data.inc` from committed JS sources -- see the `art` skill.
