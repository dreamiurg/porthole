...
Biscuit (in the launcher; its reading screens land with the next PR) is a warm, doglike reading and discovery companion on the same panel, drawn full-color at native 480x480 instead of Pets Club's indexed look. Its product contract, module map and content limits: `games/biscuit/CLAUDE.md`.
...
- `games/biscuit/`: the RGB565 game. Rules (`pet.h`), save, legacy migration, content, art, `game.h`/`game.cpp` and the Home, care, fetch, tricks and naming screens are landed; the reading and discovery screens are still landing. Brief: `games/biscuit/CLAUDE.md`.
