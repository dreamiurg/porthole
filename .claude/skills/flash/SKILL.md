---
name: flash
description: Paw Street (apps/paw-street). Use when building, flashing, or bringing up Paw Street firmware on the physical Waveshare board. Covers the exact build/upload/serial sequence, the PlatformIO-venv Python-dependency caveat, and how to set the clock.
---

1. Build: `make -C apps/paw-street firmware` (runs `pio run -e firmware` in the app dir; the board config extends the shared `platform/waveshare-round.ini`).
...
5. Monitor: `make -C apps/paw-street monitor` (or with `PORT=<port>`). Expect `[paw-street] boot`, then within a couple seconds `[paw-street] houses=N now=<epoch> heap=<bytes>`.
