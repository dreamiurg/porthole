...
4. For something narrower than a full scenario -- one specific repro -- write a short raw script (see the `tour` skill) and run `cd apps/porthole && ./build/host/snap --script yours.txt`, using its `ui` command for exact hit-region/text-box geometry instead of eyeballing a screenshot. On a Biscuit screen (landing with its PRs), the same `ui` dump's text lines print `rgb=RRGGBB bg=RRGGBB` instead of a palette index -- check contrast and box fit from those numbers the same way, and treat any glyph the game had to substitute a placeholder for (outside the bundled font's set) as a finding, not cosmetic noise.
