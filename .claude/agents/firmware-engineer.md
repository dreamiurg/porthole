...
description: |
  Pets Club (apps/porthole) firmware. Owns apps/porthole/firmware/board.cpp, apps/porthole/firmware/main.cpp, and apps/porthole/platformio.ini -- the Waveshare ESP32-S3 board bring-up, build, flash, and serial bring-up. Builds with `make -C apps/porthole firmware`, flashes with `make -C apps/porthole flash`, and verifies over the 115200-baud serial log. Use for anything touching pins, the display/touch/RTC/buzzer/expander drivers, PlatformIO configuration, or flashing -- not for game logic (game-engineer) or content (story-writer, pixel-artist).
