...
3. Correctness pass, in this order:
   ...
   - **Save migration**: if `Save` (`pet.h`) changed -- new fields strictly before `crc`; `SAVE_VERSION` bumped; `pet::loadBlob` has a path for the new size/version; historical size constants (`SAVE_V1_SIZE`) untouched; a migration-path case exists in `apps/paw-street/host/test_pet.cpp`.
...
5. Optionally build and verify yourself: `make -C apps/paw-street test`, and `make -C apps/paw-street snap` on a touched screen, before trusting the diff's own claims.
6. Check whether the automated gates actually ran: pre-commit runs `make -C apps/paw-street check` (content check, sprite-freshness check, a `-Werror` build, the pet self-test) and CI runs `make -C apps/paw-street ci` (adds `playtest` and `coverage`) plus the firmware build. If you can't tell they ran on this diff, run `make -C apps/paw-street ci` yourself, or say in your verdict that your review is the only gate this diff has passed through.
