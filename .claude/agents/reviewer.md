...
3. Correctness pass, in this order:
   ...
   - **Save migration**: if `Save` (`pet.h`) changed -- new fields strictly before `crc`; `SAVE_VERSION` bumped; `pet::loadBlob` has a path for the new size/version; historical size constants (`SAVE_V1_SIZE`) untouched; a migration-path case exists in `apps/porthole/host/test_pet.cpp`.
